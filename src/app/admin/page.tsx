...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<keyof typeof TAB>("orders");
      ...
      <header className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-extrabold" style={{ color: accent }}>{cur.name}</h1>
            <span className="rounded-full bg-white/10 px-2.5 py-1 text-[11px] font-black text-[#a6a6a3]">{PLAN_INFO[cur.plan]?.label ?? cur.plan}</span>
          </div>
          <a href={`/${cur.slug}`} target="_blank" className="text-xs text-[#a6a6a3] underline">عرض المنيو /{cur.slug}</a>
        </div>
        <div className="flex items-center gap-2">
          {rests.length > 1 && (
            <select
              ...
              className="rounded-xl border border-white/15 bg-[#1b1b1e] px-3 py-2 text-sm font-bold"
            >
              {rests.map((r) => <option key={r.id} value={r.id}>{r.name}</option>)}
            </select>
          )}
          <button onClick={() => loadData(cur)} className="rounded-xl border border-white/15 px-3 py-2 text-sm font-bold">تحديث</button>
          <button onClick={() => client.auth.signOut().then(() => router.replace("/sign-in"))} className="rounded-xl border border-white/15 px-3 py-2 text-sm font-bold">خروج</button>
        </div>
      </header>

      <nav className="mb-4 flex flex-wrap gap-2">
        {(Object.keys(TAB) as (keyof typeof TAB)[]).map((k) => (
          <button
            key={k}
            onClick={() => setTab(k)}
            className={`rounded-xl px-4 py-2 text-sm font-extrabold ${tab === k ? "text-[#0d0d0d]" : "border border-white/15 text-[#f2f2f0]"}`}
            style={tab === k ? { background: accent } : undefined}
          >
            {TAB[k]}
            {k === "orders" && orders.some((o) => o.status === "pending") ? ` (${orders.filter((o) => o.status === "pending").length})` : ""}
            {k === "bookings" && bookings.some((b) => b.status === "pending") ? ` (${bookings.filter((b) => b.status === "pending").length})` : ""}
          </button>
        ))}
      </nav>
